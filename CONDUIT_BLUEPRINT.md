...
- [ ] Set up LLM client (Gemini API + Groq API)
- [ ] Build onboarding wizard UI (multi-step: URL → upload → form → review)
- [ ] Implement Scraper Agent (Firecrawl + Jina Reader)
- [ ] Implement Document Analyst Agent (PDF/doc parsing)
- [ ] Implement Identity Synthesizer Agent (merge all sources → Brand Manifesto)
- [ ] Build Brand Manifesto editor page (view + edit JSONB data)
- [ ] Build Voice & Tone settings page (tone spectrum sliders, style dropdowns)
- [ ] Create API routes: `/api/onboarding/*`, `/api/brand`

### Phase 3 — Strategy & Calendar
- [ ] Implement Strategy Agent (pillars, schedule, themes, calendar generation)
- [ ] Build Content Strategy editor page
- [ ] Build Calendar page with drag-drop (week/month views)
- [ ] AI-suggested posting times based on platform best practices
- [ ] Create API routes: `/api/strategy/*`

### Phase 4 — Content Generation
- [ ] Implement LinkedIn Writer Agent
- [ ] Implement Instagram Writer Agent
- [ ] Implement Facebook Writer Agent
- [ ] Implement X (Twitter) Writer Agent
- [ ] Implement GBP Writer Agent
- [ ] Build content generation page (select platform, pillar → generate)
- [ ] Build draft editor with live preview (per-platform rendering)
- [ ] A/B/C variant generation (compare variants side-by-side)
- [ ] Hashtag suggestion engine
- [ ] Create API routes: `/api/content/generate`, `/api/drafts/*`

### Phase 5 — Visual Content
- [ ] Implement Visual Designer Agent
- [ ] AI image generation integration (Gemini or external)
- [ ] Carousel builder (slide editor, reorder, preview)
- [ ] Story template editor
- [ ] Image upload to R2 + linking to drafts

### Phase 6 — Approval Workflow
- [ ] Build submit-for-review flow (creator → approver)
- [ ] Build approval queue page (filterable, sortable)
- [ ] Revision request with notes UI
- [ ] Audit trail display (timeline of actions per draft)
- [ ] Notification triggers on state transitions
- [ ] Create API routes: `/api/drafts/[id]/submit|approve|revise`

### Phase 7 — Publishing
- [ ] Build platform connection setup page (guided API key entry)
- [ ] Encrypt + store platform tokens
- [ ] Implement Publisher Agent (post to each platform's API)
- [ ] Implement Scheduler Agent (queue management, optimal times)
- [ ] Post status tracking (scheduled → published / failed)
- [ ] Retry logic for failed posts
- [ ] Create API routes: `/api/platforms/*`

### Phase 8 — Analytics
- [ ] Implement Analytics Agent (pull metrics from platform APIs)
- [ ] Build analytics dashboard with charts (chart library TBD)
- [ ] Per-post performance breakdown
- [ ] A/B variant comparison view
- [ ] Trend charts (weekly/monthly)
- [ ] Create API routes: `/api/analytics/*`

### Phase 9 — Optimization & Competitors
- [ ] Implement Optimizer Agent (analyze trends → generate proposals)
- [ ] Build optimization proposals page (review, approve, reject)
- [ ] Implement Competitor Agent (discover + track competitors)
- [ ] Build competitor analysis page
- [ ] Create API routes: `/api/proposals/*`, `/api/competitors/*`
...
- [ ] Auth middleware (protect dashboard routes, redirect unauthenticated)
- [ ] Documentation
