...
import type { TrustChannelId } from "@/content/landing";

function IconFrame({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span
      className={`flex size-10 shrink-0 items-center justify-center rounded-xl bg-white/12 ring-1 ring-white/15 ${className ?? ""}`}
    >
      {children}
    </span>
  );
...
function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden>
      <defs>
        <linearGradient id="hero-trust-ig" x1="0%" y1="100%" x2="100%" y2="0%">
...
const ICONS: Record<TrustChannelId, ReactNode> = {
...
export function TrustChannelIcon({ id }: { id: TrustChannelId }) {
