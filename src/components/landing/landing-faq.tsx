  ...
  return (
    <section id="faq" className="scroll-mt-20 py-16 sm:py-20" aria-labelledby="faq-heading">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        ...
        <div className="divide-y divide-border/60 rounded-2xl border border-border/80 bg-card shadow-sm">
          {section.items.map((item) => (
            <details key={item.question} className="group">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-left text-sm font-medium text-foreground hover:text-foreground [&::-webkit-details-marker]:hidden">
                {item.question}
                <ChevronDown
